...
Board::Board(const Board &other) { operator=(other); }
  ...
  if (horiz) {
    ...
    for (int c = col; c < col + length; c++) {
      if (shotBoard[row][c] == SHIP) {
        cerr << "psotionOk: horizontal collision at row = " << row
             << ", col = " << c << endl;
        return false;
      }
    }
  } else {
    if (col < 0 || col >= boardSize || row < 0 ||
        row + length - 1 > boardSize) {
    ...
    for (int r = row; r < row + length; r++) {
      if (shotBoard[r][col] == SHIP) {
        cerr << "positionOk: horizontal collision at row = " << r
             << ", col = " << col << endl;
        return false;
      }
    }
...
char Board::getShipMark(int row, int col) { return shipBoard[row][col]; }
...
char Board::processShot(int row, int col) {
  ...
  default:
    cerr << "This didn't happen!!!! [" << row << "][" << col << "] had value "
