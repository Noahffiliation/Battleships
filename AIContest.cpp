...
AIContest::AIContest(Player* player1, string player1Name, Player* player2, string player2Name, int boardSize, bool silent) {
    // Set up player 1
...
void AIContest::showBoard(Board* board, bool ownerView, string playerName, bool fullRedraw, Side side, bool hLMostRecentShot, int hLRow, int hLCol) {
    if (silent) {
        return;
    }
...
bool AIContest::processShot(string playerName, Player* player, Board* board, Side side, int row, int col, Player* otherPlayer) {
    bool won = false;
    int resultsRow = 16;
    ...
    if (!silent) cout << gotoRowCol(resultsRow, shotColOffset) << playerName << "'s shot: [" << row << ", " << col << "]" << endl;
    ...
    switch (msg.getMessageType()) {
        ...
        case KILL:
            if (!silent) {
                cout << gotoRowCol(resultsRow + 1, boardColOffset) << setw(30) << "";
                cout << gotoRowCol(resultsRow + 1, boardColOffset) << "It's a KILL!" << msg.getString() << flush;
            }
            // Notify that it's a hit
            msg.setMessageType(HIT);
            player->update(msg);
            msg.setMessageType(KILL);
            // Notify that all segments of ship are now a KILL
        ...
        case INVALID_SHOT:
            if (!silent) {
                cout << fgColor(RED) << bgColor(WHITE);
                cout << gotoRowCol(resultsRow + 1, boardColOffset) << setw(30) << "";
                cout << gotoRowCol(resultsRow + 1, boardColOffset) << playerName << "Invalid coordinates: [row = " << row << ", col = " << col << "]" << flush;
                cout << resetAll() << flush;
            }
            player->update(msg);
            break;
        default:
            if (!silent) {
                cout << fgColor(RED) << bgColor(WHITE);
                cout << gotoRowCol(resultsRow + 1, boardColOffset) << setw(30) << "";
                cout << gotoRowCol(resultsRow + 1, boardColOffset) << "Invalid return from processShot: " << msg.getMessageType() << "(" << msg.getString() << ")";
                cout << resetAll() << flush;
            }
            player->update(msg);
            break;
...
void AIContest::play(float secondsDelay, int& totalMoves, bool&player1Won, bool& player2Won) {
    ...
    while (!(player1Won || player2Won) && totalMoves < maxShots) {
        ...
        if (!silent && secondsDelay > 0) snooze(secondsDelay);
    }

    // Show final board results
    if (!silent) {
        clearScreen();
        showBoard(player1Board, true, "Final status of " + player1Name + "'s board", true, Left, false, -1, -1);
        cout << endl;
        showBoard(player2Board, true, "Final status of " + player2Name + "'s board", true, Right, false, -1, -1);
        cout << endl;
    }
    ...
    // Show shot stats
    cout << "--- (Moves = " << totalMoves << ", percentage of board shot at = " << (100.0 * (float) totalMoves) / (boardSize * boardSize) << "%.)" << endl;
    cout << endl;
    if (!silent) snooze(5);
}
